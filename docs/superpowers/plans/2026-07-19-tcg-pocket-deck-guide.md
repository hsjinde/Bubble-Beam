...
- [ ] **Step 3: 寫 `CardImage.tsx`（含兩層 fallback：索引缺卡、圖載入失敗）**
...
```bash
git add src/components/guide/TierBadge.tsx src/components/guide/EnergyIcon.tsx src/components/guide/CardImage.tsx
git commit -m "feat: add guide primitives (tier badge, energy icon, card image)"
```
