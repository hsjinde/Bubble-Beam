...
**兩組篩選的區隔**：`/decks` 現在有兩個搜尋框、兩組 Tier 晶片。差異靠三件事——
策展那組有外框面板、標籤明講範圍（「搜尋精選攻略」vs「搜尋排行榜」）、面板頂端
一句「與上方排行榜的篩選各自獨立」。`role="group"` 的 `aria-label` 也分開了。
注意 `role="group"` 只包 Tier 按鈕，**不含「展開全部」**（那不是篩選條件）。
