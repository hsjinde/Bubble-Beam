...
- `npm run lint` 有 **171,676** 個錯誤，全是 prettier 的 `Delete ␍`（2026-07-23 實測；
  舊記錄寫 717 已過期）。`.gitattributes`（`* text=auto eol=lf`）**已經加了**（commit `a35111f`），
  但工作區從沒 renormalize 過，所以簽出仍是 CRLF。要根治得跑
  `git add --renormalize .` ＋ `npm run format`。diff 會很大但應該純粹是行尾變化。
  **目前 lint 是沒有訊號的關卡——永遠紅燈，真錯誤會被淹沒。**
