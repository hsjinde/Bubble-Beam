...
const decks: Deck[] = [
  ...
  },
  {
    ...
    strategy: [
      ...
      "注意事項：",
      "- 這套牌的固定班底非常明確：胡帕ex、達克萊伊ex、Mega阿勃梭魯ex、赤日、模仿少女、博士的研究與兩張欺瞞之針，在取樣牌表中都是 100% 採用；除障器（Field Blower）與精靈球都是 95%、兩張幸運冰棒 85%、開始的平原 80%。抄牌時這一段不要動。",
      "- 真正的自由席只剩一格：X速度（35%）。小氣球 70%、Repel 65% 與 Pokémon Center Lady 55% 屬於多數牌表會帶的常見牌，這幾張加上 X速度才是你按環境調整的空間。",
  ...
  },
  {
    ...
    strategy: [
      ...
      "- 自由席比想像中多：優雅披風（Elegant Cape）90%、除障器 80%、訓練場（Training Area）70%，落雷獸則在 B2b 版（65%）與 promo 版（40%）之間各家取捨。這代表牌表細節可以按手感調整，但兩條進化線的「終點」不能少。",
