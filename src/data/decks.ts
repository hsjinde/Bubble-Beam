...
const decks: Deck[] = [
  ...
  },
  {
    ...
    strategy: [
      ...
      "注意事項：",
      "- 這套牌的固定班底非常明確：胡帕ex、達克萊伊ex、Mega阿勃梭魯ex、赤日、模仿少女、精靈球、博士的研究、除障器（Field Blower）、兩張欺瞞之針與兩張幸運冰棒，在取樣牌表中都是 100% 採用；開始的平原 85%。抄牌時這一段不要動。",
      "- 真正的自由席只剩兩格：小氣球（55%）與部分牌表改帶的 X速度（40%）——Repel 已經漲到 90% 採用，實質上跟固定班底同級。這兩格才是你按環境調整的空間。",
  ...
  },
  {
    ...
    strategy: [
      ...
      "- 攻擊手的搭配是這套牌最大的變數：碧草面具厄鬼椪ex 幾乎人人都帶（90%），三蜜蜂 90%、莉佳 60%，而費洛美螂（45%）與紙御劍（35%）則是明顯的自由席，看你要對速攻還是對重砲。",
  ...
  },
  {
    ...
    strategy: [
      ...
      "- 核心固定：呱呱泡蛙、甲賀忍蛙、Mega路卡利歐ex、神奇糖果、赤日、模仿少女、精靈球、博士的研究（100% 採用），利歐路與幸運蛋（Lucky Egg）各 85%。小果然 55%、除障器 60% 則是可調整的部分。",
