...
export function GuideEntry() {
  return (
    <Link
      ...
      className="group fixed bottom-8 left-4 z-40 flex w-[10.5rem] -rotate-3 flex-col gap-1 rounded-[0.35rem] bg-white/75 px-4 py-3.5 shadow-[0_10px_26px_rgba(42,111,151,0.28)] backdrop-blur-sm transition duration-300 hover:-translate-y-1 hover:rotate-0 hover:bg-white/90 sm:bottom-10 sm:left-8 sm:w-[14rem] sm:px-5 sm:py-5"
      style={{ fontFamily: "'Comic Sans MS', 'Segoe Print', cursive" }}
    >
      {/* 膠帶 */}
      <span
        className="absolute left-1/2 top-0 h-4 w-16 -translate-x-1/2 -translate-y-1/2 -rotate-[4deg] rounded-[2px] bg-guide-tint/80 sm:h-5 sm:w-20"
      ...
      <span className="self-end text-sm text-guide-ink-deep transition-transform duration-300 group-hover:translate-x-1 sm:text-base">
        →
      </span>
    </Link>
  );
